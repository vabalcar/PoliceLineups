...
class ServiceScriptExecutor : ScriptExecutor {
    [void] Execute([ScriptExecution[]] $scriptExcutions) {
        ...
        | ForEach-Object {
            ...
            $serviceProcess = Start-Process -PassThru @windowStyle -WorkingDirectory $_.WD -Path 'pwsh' -ArgumentList '-NoLogo', '-NonInteractive', '-Command', "& pwsh -File $($_.Script) $($_.ArgumentList) *> $logFile"
